#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 30 18:44:22 2023

@author: daniyal
"""
import cv2 as cv
import pygad
import numpy as np
import matplotlib.pyplot as plt
import skimage
from datetime import datetime
from scipy.ndimage import generic_filter
import math

n = 3
filter_size = (n, n)
w = filter_size[0]//2
x = (1 - filter_size[0]%2) - (filter_size[0]//2)
y = filter_size[1]//2
z = (1 - filter_size[1]%2) - (filter_size[1]//2)



image = cv.imread("../img/balloons_lowcontrast_lowbrightness.png", 0)
image1 = np.copy(image)

def Func(a):
    return np.prod( np.power(a, 1/9))

def calc_global_mean(img):
    r, c = np.shape(img)
    return np.sum(img)/(r*c)

def func1(a):
    return math.sqrt(np.sum(a**2))

def std_dev(img):
    
    # Convert the image to a floating-point array
    img = np.array(img, dtype=np.float32)
    
    # Pad the image with zeros to preserve the size of the output
    img = np.pad(img, ((1, 1), (1, 1)), mode='constant')
    
    # Create a 3x3 filter with all elements set to 1/9
    filter = np.ones((3, 3), dtype=np.float32) / 9
    
    # Calculate the mean image using a convolution
    mean = cv.filter2D(img, -1, filter, borderType=cv.BORDER_CONSTANT)
    
    # Trim the padding to obtain the original size of the image
    mean = mean[1:-1, 1:-1]
    
    # Subtract the mean from the original image
    img = img[1:-1, 1:-1] - mean
    
    # Square the result to get the squared differences
    squared_differences = img**2
    
    # Calculate the local mean of the squared differences
    mean_squared_differences = cv.filter2D(squared_differences, -1, filter, borderType=cv.BORDER_CONSTANT)
    
    # Calculate the standard deviation as the square root of the local mean of the squared differences
    std_deviation = np.sqrt(mean_squared_differences)
    
    # Normalize the result to the range [0, 255]
    return np.uint8(255 * std_deviation / np.max(std_deviation))
    
    


def local_am(img):
    # Convert the image to a floating-point array
    img = np.array(img, dtype=np.float32)
    
    # Pad the image with zeros to preserve the size of the output
    img = np.pad(img, ((1, 1), (1, 1)), mode='constant')
    
    # Create a 3x3 filter with all elements set to 1/9
    filter = np.ones((3, 3), dtype=np.float32) / 9
    
    # Calculate the mean image using a convolution
    mean = cv.filter2D(img, -1, filter, borderType=cv.BORDER_CONSTANT)
    
    # Trim the padding to obtain the original size of the image
    mean = mean[1:-1, 1:-1]
    
    # Convert the result back to 8-bit unsigned integers
    mean = np.uint8(mean)
    return mean


arith_mean_arr = local_am(image)

geom_mean_arr = generic_filter(image, Func, size=filter_size)[w:x or None, y:z or None]
std_dev_arr = std_dev(image)
global_mean = calc_global_mean(image)
   
def calculateGain(img, b, k, i, j, n):
    imgH, imgW =  np.shape(img)
    return k*global_mean/(std_dev_arr[i][j]+b)

# am(image)
# gm(image)
 
def imageTransform(img, a, b, c, k):
    imgH, imgW =  np.shape(img)
    temp = np.zeros(np.shape(img), dtype="float32")
    global arith_mean_arr
    for i in range(0, imgH):
        for j in range(0, imgW):
            m = arith_mean_arr[i, j]
            gain = calculateGain(img, b, k, i, j, 3)
            temp[i][j] = gain*(img[i][j]-c*m) + m**a
            
    # print("max val in image : {m}".format(m = np.amax(temp)))
    # max_val = np.amax(temp)
    # if max_val>255:
    #     med = np.median(temp)
    #     diff = med - 128
    #     temp = temp - diff
        
    temp = np.clip(temp, 0, 255)
    temp = temp.astype('uint8')
    return temp



def calculate_HFM_HS(im):
    L = 256
    hist = cv.calcHist([im],[0],None,[256],[0,256])
    hs = (np.percentile(hist, 75) - np.percentile(hist, 25))/255
    r = np.power(hist, 1/L)
    g = np.prod(r, where = r > 0)  
    a = np.sum(hist)/L
    hfm =  g/a
    return hfm, hs



# orig_exposure = calculate_exposure(image)
# p = 1 if orig_exposure < 0.5 else -1

def produce_edge_image(img):
    # Apply the Sobel operator to the image
    sobelx = cv.Sobel(img, cv.CV_64F, 1, 0, ksize=5)
    sobely = cv.Sobel(img, cv.CV_64F, 0, 1, ksize=5)
    
    # Combine the results of the two Sobel operations
    sobel_combined = np.hypot(sobelx, sobely)
    
    # Normalize the result to the range [0, 255]
    sobel_combined = np.uint8(255 * sobel_combined / np.max(sobel_combined))
    
    # Threshold the result to create a binary edge map
    _, thresholded = cv.threshold(sobel_combined, 128, 255, cv.THRESH_BINARY)
    return thresholded

def fitness_function(solution, solution_idx):
    a, b, c, k = solution
    global image, p
    m, n = np.shape(image)
    
    enhanced = imageTransform(np.copy(image), a, b, c, k)
    
    entropy = skimage.measure.shannon_entropy(enhanced)
    
    edge_img = produce_edge_image(enhanced)
    edge_map = np.array(edge_img)
    eI_s = np.sum(edge_map)
    # Count the number of non-zero pixels
    # num_edgels = np.count_nonzero(edge_map)
    num_edgels = (edge_map>150).sum()
    print(eI_s)
    # if eI_s>0:
    #     fitness = entropy * math.log(math.log(eI_s, 2), 2) * num_edgels/(m*n)
    # else:
    #     fitness = 0
    return entropy*num_edgels/(m*n)


last_fitness = 0
count = 0
def on_generation(ga_instance):
    global last_fitness
    print("Generation = {generation}".format(generation=ga_instance.generations_completed))
    print("Fitness    = {fitness}".format(fitness=ga_instance.best_solution(pop_fitness=ga_instance.last_generation_fitness)[1]))    
    print("Change     = {change}".format(change=ga_instance.best_solution(pop_fitness=ga_instance.last_generation_fitness)[1] - last_fitness))
    last_fitness = ga_instance.best_solution(pop_fitness=ga_instance.last_generation_fitness)[1]
    # if ga_instance.best_solution(pop_fitness=ga_instance.last_generation_fitness)[1] - last_fitness==0:
    #     return "stop"

a__ = np.linspace(0.0, 1.5, 500, endpoint=True)
c__ = np.linspace(0, 5, 500, endpoint=True)

ga_instance = pygad.GA(num_generations=30,
                        num_parents_mating=3, 
                        fitness_func=fitness_function,
                        # init_range_high=1,
                        # init_range_low=0.0,
                        sol_per_pop=10, 
                        num_genes=4,
                        mutation_type="random",
                        mutation_by_replacement=True,
                        mutation_num_genes=1,
                        mutation_percent_genes=0.01,
                        # random_mutation_max_val=1,
                        # random_mutation_min_val=0,
                        # gene_space=[a__, c__],
                        on_generation=on_generation
                        )


ga_instance.run()

ga_instance.plot_fitness()

solution, solution_fitness, solution_idx = ga_instance.best_solution()
print("Fitness value of the best solution = {solution_fitness}".format(solution_fitness=solution_fitness))
print("Index of the best solution : {solution_idx}".format(solution_idx=solution_idx))

if ga_instance.best_solution_generation != -1:
    print("Best fitness value reached after {best_solution_generation} generations.".format(best_solution_generation=ga_instance.best_solution_generation))

print("optimal a:{a} and c:{c}".format(a=solution[0], c= solution[1]))


a, b, c, k = solution

final_image = imageTransform(image, a, b, c, k)

cv.imwrite("res_{now}.png".format(now=datetime.now()), final_image)

fig = plt.figure(figsize=(6, 6))

fig.add_subplot(221)
plt.imshow(image, cmap='gray')


fig.add_subplot(222)
plt.imshow(final_image, cmap='gray')

fig.add_subplot(223)
plt.hist(image.ravel(), 256,[0,256])
plt.show()

fig.add_subplot(224)
plt.hist(final_image.ravel(), 256,[0,256]); 
plt.show()


cv.waitKey(0)
cv.destroyAllWindows()


